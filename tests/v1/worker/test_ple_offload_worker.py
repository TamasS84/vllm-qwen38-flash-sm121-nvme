...
class _WeightLoadingPleLayer(_TestPleOffloadLayer):
    def __init__(self, preload_weight: bool = False) -> None:
        super().__init__()
        self.preload_weight = preload_weight
        self.weight = torch.nn.Parameter(torch.zeros(2))
        self.bias = torch.nn.Parameter(torch.zeros(2))

    def is_preloaded_checkpoint_weight(self, relative_name: str) -> bool:
        return self.preload_weight and relative_name in {
            "shard_0.weight",
            "shard_1.weight",
        }

    def preloaded_parameter_names(self) -> set[str]:
        return {"weight"} if self.preload_weight else set()
...
class _WeightLoadingModel(torch.nn.Module):
    ...
    def __init__(self, preload_weight: bool = False) -> None:
        super().__init__()
        self.ple = _WeightLoadingPleLayer(preload_weight)
...
def test_ple_offload_skips_preloaded_checkpoint_shards(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    with caplog.at_level("INFO", logger=ple_offload_worker.__name__):
        runner, model = _load_test_ple_weights(
            monkeypatch,
            [
                "checkpoint.ple.shard_0.weight",
                "checkpoint.ple.shard_1.weight",
                "checkpoint.ple.bias",
            ],
            preload_weight=True,
        )
    ...
    assert runner.layer_names == ["ple"]
    assert model.received_checkpoint_names == ["checkpoint.ple.bias"]
    assert "skipped 2 preloaded checkpoint tensor(s)" in caplog.text
    assert "verified 2/2 materialized parameter(s)" in caplog.text


def test_ple_offload_still_rejects_unrelated_missing_preloaded_parameters(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with pytest.raises(RuntimeError, match=r"parameters: \['ple.bias'\]"):
        _load_test_ple_weights(
...
def test_ple_nvme_path_is_exposed_from_environment(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("VLLM_PLE_NVME_PATH", "/ple/ple.fp8")

    assert envs.VLLM_PLE_NVME_PATH == "/ple/ple.fp8"


def test_worker_rejects_nvme_without_cpu_offload(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(envs, "VLLM_PLE_NVME_PATH", "/ple/ple.fp8", raising=False)
    monkeypatch.setattr(envs, "VLLM_PLE_CPU_OFFLOAD", False)

    with pytest.raises(ValueError, match="VLLM_PLE_CPU_OFFLOAD=1"):
        gpu_worker_module._validate_ple_nvme_environment()
...
@pytest.mark.parametrize(
    ...
    [
        ("Qwen4ExpForCausalLM", False, "mp", None),
        ("Qwen4ExpForConditionalGeneration", True, "mp", None),
        ("UnsupportedArchitecture", True, "mp", "architecture"),
        ("Qwen4ExpForCausalLM", False, "uni", "executor backend=uni"),
    ],
)
...
def test_offload_handle_reports_unexpected_child_exit() -> None:
    class ExitedProcess:
        exitcode = 17
    ...
    handle.start_monitor(exit_codes.append)
    assert handle.monitor_thread is not None
    handle.monitor_thread.join(timeout=1)

    assert exit_codes == [17]


def test_offload_handle_does_not_report_requested_shutdown() -> None:
    class ExitedProcess:
        exitcode = 0
...
def test_gpu_worker_aborts_when_offload_child_exits(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    callbacks = []
    exit_codes = []
    ...
    worker = Worker.__new__(Worker)
    worker._ple_offload_worker_handle = FakeHandle()
    monkeypatch.setattr(
        ple_offload_worker.PleOffloadWorker,
        "wait_for_ready",
        lambda _: None,
    )
    monkeypatch.setattr(gpu_worker_module.os, "_exit", exit_codes.append)

    worker.wait_ple_offload_ready()
