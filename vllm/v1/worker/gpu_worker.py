...
from vllm.model_executor.layers.ple_nvme import validate_ple_nvme_config
...
def _validate_ple_nvme_environment() -> None:
    """Reject invalid NVMe PLE settings before model initialization."""
    validate_ple_nvme_config(
        envs.VLLM_PLE_NVME_PATH,
        cpu_offload_enabled=envs.VLLM_PLE_CPU_OFFLOAD,
    )
...
class Worker(WorkerBase):
    def __init__(
        ...
        _validate_ple_nvme_environment()
    ...
    def wait_ple_offload_ready(self) -> None:
        ...
        self._ple_offload_worker_handle.start_monitor(
            self._abort_for_ple_offload_failure
        )

    @staticmethod
    def _abort_for_ple_offload_failure(exit_code: int | None) -> None:
        logger.error(
            "PLE offload worker exited unexpectedly (exit code: %s); "
            "terminating the GPU worker to avoid a stalled CUDA semaphore.",
            exit_code,
        )
        os._exit(1)
