...
class PleOffloadWorkerHandle:
    ...
    shutdown_requested: threading.Event = field(
        default_factory=threading.Event, init=False, repr=False
    )
    monitor_thread: threading.Thread | None = field(
        default=None, init=False, repr=False
    )

    def start_monitor(
        self,
        failure_callback: Callable[[int | None], None],
    ) -> None:
        """Report an unexpected child exit to the owning GPU worker."""
        ...
        def monitor() -> None:
            ...
            if not self.shutdown_requested.is_set():
                failure_callback(self.proc.exitcode)

        self.monitor_thread = threading.Thread(
            target=monitor,
            daemon=True,
            name="PleOffloadWorkerMonitor",
        )
...
class PleOffloadRunner:
    ...
    def _load_weights(self) -> None:
        ...
        skipped_preloaded_tensors = 0

        def offload_only_iter(
            weights: Iterable[tuple[str, torch.Tensor]],
        ) -> Iterable[tuple[str, torch.Tensor]]:
            nonlocal matched_checkpoint_tensors, skipped_preloaded_tensors
            for weight_name, tensor in weights:
                ...
                if mapped_name is not None and mapped_name.startswith(offload_prefixes):
                    ...
                    for layer_name, layer in offload_layers.items():
                        ...
                        if not mapped_name.startswith(layer_prefix):
                            continue
                        relative_name = mapped_name[len(layer_prefix) :]
                        if layer.is_preloaded_checkpoint_weight(relative_name):
                            skipped_preloaded_tensors += 1
                            break
                    else:
        ...
        if isinstance(loader, DummyModelLoader):
            ...
            preloaded_offload_params = {
                f"{layer_name}.{param_name}"
                for layer_name, layer in offload_layers.items()
                for param_name in layer.preloaded_parameter_names()
            }
            accounted_params = set(loaded_params).union(preloaded_offload_params)
            loaded_offload_entries = {
                name for name in loaded_params if name.startswith(offload_prefixes)
            }
            loaded_expected_params = expected_offload_params.intersection(
                accounted_params
            )
            ...
            logger.info(
                "PLE offload matched %d checkpoint tensor(s), skipped %d "
                "preloaded checkpoint tensor(s), loaded %d "
                "offload entries, and verified %d/%d materialized "
                "parameter(s) for layers: %s",
                matched_checkpoint_tensors,
                skipped_preloaded_tensors,
